...
<div class="row form-group">
    <label for="preciod" class="col-5 text-white">Precio</label>
    <input type="number" wire:model="preciod" placeholder="0.00"  min="0" step="0.01" pattern="^\d+(?:\.\d{1,2})?$" onblur="this.parentNode.parentNode.style.backgroundColor=/^\d+(?:\.\d{1,2})?$/.test(this.value)?inherit':'red'" class="form-control col-md-7" readonly="readonly">
    @error('preciod') <span class="text-warning">{{$message}}</span> @enderror
</div>

<div class="row form-group">
    <label for="descuentod" class="col-5 text-white">Descuento</label>
    <input type="number" wire:model="descuentod" placeholder="0.00"  min="0" step="0.01" pattern="^\d+(?:\.\d{1,2})?$" onblur="this.parentNode.parentNode.style.backgroundColor=/^\d+(?:\.\d{1,2})?$/.test(this.value)?inherit':'red'" class="form-control col-md-7" readonly="readonly">
    @error('descuentod') <span class="text-warning">{{$message}}</span> @enderror
</div>
